	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) {
				...
				if err := client.DeleteTodo(int32(id)); err != nil {
					log.Fatal(err)
					return
				}
				fmt.Println("todo", id, "deleted" )
			},
		},
		{
			Name:  "doing",
			...
			Action: func(c *cli.Context) {
				...
				todo, err := client.UpdateTodoStatus(int32(id), api.DoingStatus)
				if err != nil {
					log.Fatal(err)
					return
				}
		...
		},
		{
			...
			Action: func(c *cli.Context) {
				...
				todo, err := client.UpdateTodoStatus(int32(id), api.DoneStatus)
				if err != nil {
					log.Fatal(err)
					return
				}
		...
		{
			Name:  "show",
			Usage: "(id title description) show a todo title and description",
			Action: func(c *cli.Context) {
				idStr := c.Args().Get(0)
				id, err := strconv.Atoi(idStr)
				if err != nil {
